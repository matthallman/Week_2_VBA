...
with open(file_to_save, "w") as txt_file:
    ...
    for county in county_votes:
        ...
        vote_pct = float(c_votes) / float(total_votes) *100

         # 6d: Print the county results to the terminal.
        county_results = f"{county}: {vote_pct: .1f}% ({c_votes:,})\n" 
        ...
        if (c_votes > winning_county_count) and (vote_pct > winning_county_pct):
            winning_county_count = c_votes
            winning_county_pct = vote_pct
